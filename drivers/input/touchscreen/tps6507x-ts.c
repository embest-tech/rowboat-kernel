 ...
 *
 * Touchscreen driver for the tps6507x family of chips.
 *
				   ...
				   u8 tsc_mode, u16 *value)
{
	...
	if (!loops) {
		dev_err(tsc->dev, "ADC conversion did not start\n");
		goto err;
	}
...
static void tps6507x_ts_handler(struct work_struct *work)
{
	...
	/* TS calibration data obtained from ts_calibrate */
	int axis_table[] = {32610, -23, -1118848, -228, -19267, 18811278, 16};
#endif
...
#ifdef CONFIG_ANDROID
		xtmp = (axis_table[2] + axis_table[0]*tsc->tc.x + axis_table[1]*tsc->tc.y )
			>> axis_table[6];
		ytmp = (axis_table[5] + axis_table[3]*tsc->tc.x + axis_table[4]*tsc->tc.y )
			>> axis_table[6];
                input_report_abs(input_dev, ABS_X, xtmp);
                input_report_abs(input_dev, ABS_Y, ytmp);
...
static int tps6507x_ts_open(struct input_dev *input_dev)
{
	...
	ret = queue_delayed_work(tsc->wq, &tsc->work,
				 tsc->poll_period * HZ / 1000);
